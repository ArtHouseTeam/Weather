...
BOOL CPictureEx::Draw(CRect dlgrect)
{
	gifwidth = dlgrect.Width();
	gifheight = dlgrect.Height();
	if (!m_bIsInitialized)
	{
		TRACE(_T("Call one of the CPictureEx::Load() member functions before calling Draw()\n"));
		return FALSE;
...
void CPictureEx::OnPaint() //화면 크기에 맞게 출력되도록 수정한 함수.
{
	CRect rectcc;
	...
	{	//gif의 위치, 크기 조정를 조정하여 출력. 배경을 투명처리해서 출력.		
		::TransparentBlt(dc.m_hDC, (int)((float)gifwidth * 0.67), (int)((float)gifheight*0.24), (int)((float)gifwidth / 10), (int)((float)gifwidth / 10),
			m_hMemDC, 2,2, m_PictureSize.cx-4, m_PictureSize.cy-4, RGB(140, 140, 140));
		//::StretchBlt(dc.m_hDC, (int)((float)gifwidth * 0.67), (int)((float)gifheight*0.24), (int)((float)gifwidth/10), (int)((float)gifwidth/10 ),
			//m_hMemDC, 0, 0, m_PictureSize.cx, m_PictureSize.cy, SRCCOPY);
	};
}
/*
void CPictureEx::OnPaint() //백업.
{
	CPaintDC dc(this); // device context for painting
	LONG nPaintWidth = m_PaintRect.right - m_PaintRect.left;
